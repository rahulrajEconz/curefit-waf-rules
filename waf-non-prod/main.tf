locals {
  office_vpn_ip_chunks = chunklist(var.allowed_ip_ranges, 10)
  whitelisted_ips      = chunklist(var.whitelisted_ip_address, 10)
  blocked_ip_chunks    = chunklist(var.blocked_ip_ranges, 10)
}

resource "google_compute_security_policy" "non-prod-policy" {
...
###########################################
# 2. OfficeVpnPlusCoworkingSpaces (IP based)
###########################################
resource "google_compute_security_policy_rule" "office_vpn" {
  for_each        = var.enable_office_vpn_coworking ? { for i, chunk in local.office_vpn_ip_chunks : i => chunk } : {}
  security_policy = google_compute_security_policy.non-prod-policy.name
  ...
  description     = "${var.office_vpn_coworking_description} (chunk ${tonumber(each.key) + 1})"
...
# 6. whitelisted-ips
###########################################
resource "google_compute_security_policy_rule" "whitelisted_ips" {
  for_each        = var.enable_whitelisted_ip_address ? { for i, chunk in local.whitelisted_ips : i => chunk } : {}
  security_policy = google_compute_security_policy.non-prod-policy.name
  ...
  description     = "${var.whitelisted_ip_address_description} (chunk ${tonumber(each.key) + 1})"
...
###########################################
# 7A. Slow Rate Limit – Auth & Notifications
###########################################
...
###########################################
# 7B. Slow Rate Limit – FitnessPass APIs
###########################################
...
###########################################
# 7C. Slow Rate Limit – Cultwatch APIs
###########################################
...
###########################################
# 7D. Slow Rate Limit – Gymfit & Account APIs
###########################################
...
###########################################
# 8. Blocklist IP Address
###########################################
resource "google_compute_security_policy_rule" "blocklist_ips" {
  for_each        = var.enable_blocklist_ip_address ? { for i, chunk in local.blocked_ip_chunks : i => chunk } : {}
  security_policy = google_compute_security_policy.non-prod-policy.name
  ...
  description     = "${var.blocklist_ip_address_description} (chunk ${tonumber(each.key) + 1})"
...
###########################################
# 9. Block Malicious Keyword in Query Params
###########################################
...
###########################################
# 11. Whitelist Device ID (Header: deviceid)
###########################################
...
###########################################
# 13. Exclude specific URI paths from AWS Managed Rules
###########################################
...
###########################################
# 14. Block requests with missing or empty User-Agent header
###########################################
...
###########################################
# 15(1). Block requests from bad User-Agent bots
###########################################
...
###########################################
# 15(2). Block requests from bad User-Agent bots
###########################################
...
###########################################
# 16. Block requests with large query strings
###########################################
...
###########################################
# 17. Block requests with large Cookie headers
###########################################
...
###########################################
# 19. Block requests with long URI paths
###########################################
...
###########################################
# 20. Block requests triggering EC2 Metadata SSRF
###########################################
...
###########################################
# 42. Host header contains localhost
###########################################
...
###########################################
# 45. AWSManagedReconnaissanceList
###########################################
...
###########################################
# 51. Visibility_Endpoints_rule – URI Path
###########################################
...
###########################################
# 52. DDoS_rate_limit – IP Rate Limiting
###########################################
